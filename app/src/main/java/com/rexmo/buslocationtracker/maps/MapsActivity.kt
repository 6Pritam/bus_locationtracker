...
internal class MapsActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    var gunupur = LatLng(19.0493674,83.8324191)
   ...
   override fun onMapReady(googleMap: GoogleMap) {
        ...
        //gunupur = LatLng(19.0493674,83.8324191)
        mMap.addMarker(MarkerOptions()
            .position(gunupur)
            .title("Marker in Gunupur"))
        mMap.moveCamera(CameraUpdateFactory.newLatLngZoom(gunupur,17f))
        mMap
    }
    ...
    fun readFirestoreData() {
        ...
        dataBase.get().addOnSuccessListener {
            if (it.exists()) {
                var latitude=it.getString("latitude").toString().toDouble()
                var longitude=it.getString("longitude").toString().toDouble()
//                s =
//                    it.getString("busNO") + it.getString("longitude") + it.getString("latitude") + "\n"
                //binding.txtShowLocation.text = s
                gunupur=LatLng(latitude,longitude)
